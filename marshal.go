...
type (
	...
	// helper methods for writing custom UploUnmarshaler implementations. These
	...
	// methods for writing custom UploMarshaler implementations. All of its methods
	...
	// A UploMarshaler can encode and write itself to a stream.
	UploMarshaler interface {
		MarshalUplo(io.Writer) error
	}

	// A UploUnmarshaler can read and decode itself from a stream.
	UploUnmarshaler interface {
		UnmarshalUplo(io.Reader) error
	}
)
...
func (e *Encoder) encode(val reflect.Value) error {
	...
	// check for MarshalUplo interface first
	if val.CanInterface() {
...
func (d *Decoder) decode(val reflect.Value) {
	// check for UnmarshalUplo interface first
	if val.CanAddr() && val.Addr().CanInterface() {
		if u, ok := val.Addr().Interface().(UploUnmarshaler); ok {
			err := u.UnmarshalUplo(d)
			if err != nil {
				panic(err)
			}
			return
